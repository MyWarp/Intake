...
public interface CommandArgs {
   ...
   * <p>The current position indicates the value that will be returned by the next call to {@link #next()} and it
   * starts
   * at 0. If the current position is equal to {@link #size()}, then there are no more arguments available.</p>
   *
   ...
   * <p>Keys are the flag (case-sensitive) and values are the values for the flag. For boolean flags, the value should
   * be the string "true". Flags are commonly defined by the player by using syntax similar to {@code -X value}.</p>
   *
   ...
   * <p>The keys and values in a Namespace are defined before command parsing has begun and they can be used by commands
   * or {@link Provider} to get session-related values.</p>
   *
